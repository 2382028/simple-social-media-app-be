...
export class CareLogsService {
  ...
  private async verifyPlantOwnership(
    userId: number,
    plantId: number,
  ): Promise<Plant> {
  ...
  async findOneForUser(
    ...
    // Verifikasi kepemilikan plant
    await this.verifyPlantOwnership(userId, plantId);
    const log = await this.careLogsRepository.findOne({
      where: { id: logId, plantId: plantId },
    });
    if (!log) {
      throw new NotFoundException(
        `Care log with ID ${logId} not found for plant ${plantId}`,
      );
    }
    return log;
  }
  ...
  async remove(userId: number, plantId: number, logId: number): Promise<void> {
    ...
    await this.verifyPlantOwnership(userId, plantId);
    const result = await this.careLogsRepository.delete({
      id: logId,
      plantId: plantId,
    });
